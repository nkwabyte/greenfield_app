...
export function EditGroupDialog({ open, onOpenChange, group }: EditGroupDialogProps) {
    ...
    const form = useForm<EditGroupValues>({
        resolver: zodResolver(editGroupSchema) as any,
        defaultValues: {
            name: '',
            region: '',
            district: '',
    ...
    // Populate form when dialog opens with a valid group
    React.useEffect(() => {
        if (open && group) {
            form.reset({
                name: group.name,
    ...
    // Reset district when region changes, only if the user explicitly changed it
    React.useEffect(() => {
        if (open && group && selectedRegion !== group.region) {
            form.setValue('district', '', { shouldValidate: false });
        }
    }, [selectedRegion, form, open, group]);
                ...
                <DialogHeader>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <DialogDescription>
                                Update the details of this farmer group.
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
